            ...
            <div className={`flex gap-6 items-stretch ${side === 'left' ? 'flex-row-reverse' : 'flex-row'}`}>
                {sortedRounds.map(([roundNum, roundMatches]) => (
                    <div key={roundNum} className="flex flex-col gap-4 justify-around min-w-[260px] py-2 h-full">
                        <div className="text-[10px] font-bold text-slate-500 tracking-widest uppercase text-center mb-1 shrink-0">
                            Round {roundNum}
                        </div>
...
export default function TournamentBracket({
    ...
    return (
        <div className="space-y-6">
            ...
            {/* Team Pool */}
            {availableEntries.length > 0 && (
                <div className="sticky top-0 z-40 py-2 bg-slate-950/95 backdrop-blur-md -mx-2 px-2 sm:mx-0 sm:px-0 border-b border-white/5 shadow-2xl">
                    <Card className="p-4 border-slate-700/50 shadow-xl bg-slate-900/80">
